...
void citizen_monthlybill_curve_populate(GtkCurve *curve, const double *reservations, int num_days) {
  ...
  gdouble curve_points[num_days * 2]; // Each point has an X (day) and Y (value)
  ...
  for (int i = 0; i < num_days; i++) {
    double day_normalized = (double)i / (num_days - 1); // Normalize X (0.0 to 1.0)
    double value_normalized = reservations[i] / max_value; // Normalize Y (0.0 to 1.0)
...
void citizen_account_populate(){
  ...
  citizen_monthlybill_curve_populate(monthly_bill_curve, bills, 31);
}